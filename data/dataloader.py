...
class ExceptionWrapper(object):
	"""Wraps an exception plus traceback to comunicate across threads"""
...
def default_collate(batch):
	...
	raise TypeErroe(("batch must contain tensors, numbers or lists; found {}".format(type(batch[0]))))
...
class DataLoaderIter(object):
	...
	def __next__(self):
		...
		if self.rcvd_indx in self.reorder_dict:
			batch = self.reorder_dict.pop(self.rcvd_indx)
	...
	def __getstate__(self):
		...
		# to a seperate thread and then just sharing the data queue
...
class DataLoader(object):
	...
	Arguments:
		...
		batch_size (int, optional): how many samples per batch to load
			(default: 1).
		shuffle (boll, optional): set to ``True`` to have the data reshuffled
